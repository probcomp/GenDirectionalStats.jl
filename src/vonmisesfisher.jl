...
log_besseli_term(m, x) = -logfactorial(m) - loggamma(m + 2) + (2*m + 1)*log(x/2)
...
function _log_besseli_1(x; n_terms=100)
    log_terms = Vector{Float64}(undef, n_terms)
    for m in 0:n_terms-1
        log_terms[m+1] = log_besseli_term(m, x)
    end
    return logsumexp(log_terms)
end

const max_lookup_input = 50000
const log_besseli_lookup = Vector{Any}(undef, max_lookup_input)

function log_besseli_1(x; n_terms="auto")
    @assert x < max_lookup_input
    if n_terms == "auto"
        n_terms = Int(floor(x/2)) + 20
    end
    if x > 500.0
        floorx = Int(floor(x))
        if !isassigned(log_besseli_lookup)
            log_besseli_lookup[floorx] = _log_besseli_1(floorx; n_terms=n_terms)
        end
        return log_besseli_lookup[floorx]
    else
        return _log_besseli_1(x; n_terms=n_terms)
    end
end
...
function _vmflck(p, κ)
    ...
    hp = T(p/2)
    q = hp - 1
    logbessel_term = q == 1 ? log_besseli_1(κ) : log(besseli(q, κ))
...
function test_logbesseli_1(z)
    actual = log_besseli_1(z; n_terms=10)
    expected = log(besseli(1, z))
    @show actual, expected
    actual = log_besseli_1(z; n_terms=100)
    expected = log(besseli(1, z))
    @show actual, expected
    actual = log_besseli_1(z; n_terms=1000)
    expected = log(besseli(1, z))
    @show actual, expected
end

# @show log_besseli_1(4001; n_terms=10)
# @show log_besseli_1(4001; n_terms=100)
# @show log_besseli_1(4001; n_terms=1000)
# @show log_besseli_1(4001; n_terms=10000)
# @show log_besseli_1(4001; n_terms=100000)
...
function test_logbesseli_2(x)
    ...
    while abs(approx - prev_approx) > 0.01
        prev_approx = approx
        approx = _log_besseli_1(x; n_terms=n_terms)
        n_terms += 1
    end
    return n_terms
end
